...
### 1-7-12. マージしたmainをGitHubへプッシュする
...
スカッシュ（squash）とは、複数のコミットを1つのコミットにまとめることである。

例えば、作業中に次のコミットを作成したとする。
...
これらをスカッシュすると、次のような1つのコミットにまとめられる。
...
このまま`main`ブランチへ統合すると、履歴が細かくなり、何のための変更だったのか分かりにくくなる。

スカッシュを使うと、関連するコミットを1つにまとめられるため、`main`ブランチの履歴を見やすくできる。
...
通常のマージでは、作業用ブランチにある各コミットを残したまま、`main`ブランチへ統合する。
...
それぞれの作業履歴を詳しく確認できる一方で、コミット数が多くなる場合がある。

### スカッシュマージ

スカッシュマージでは、作業用ブランチにある複数のコミットを1つにまとめてから、`main`ブランチへ統合する。

```text
ログイン機能を追加
```

細かい修正履歴がまとめられるため、`main`ブランチの履歴が分かりやすくなる。

---

## GitHubの「Squash and merge」

GitHubのプルリクエストでは、次のようなマージ方法を選択できる。

- `Create a merge commit`
- `Squash and merge`
- `Rebase and merge`

`Squash and merge`を選ぶと、作業用ブランチにある複数のコミットをGitHubが1つにまとめてから、`main`ブランチへ追加する。
...
コマンドを使って、直近3個のコミットをまとめる場合は次のように入力する。

```bash
git rebase -i HEAD~3
```

実行すると、直近3個のコミットがエディタに表示される。

```text
pick コミット番号 1つ目のコミット
pick コミット番号 2つ目のコミット
pick コミット番号 3つ目のコミット
```

最初のコミットは`pick`のままにし、まとめたいコミットを`squash`または`s`へ変更する。

```text
pick コミット番号 1つ目のコミット
squash コミット番号 2つ目のコミット
squash コミット番号 3つ目のコミット
```

保存すると、3個のコミットが1つにまとめられる。

ただし、`git rebase`は過去のコミット履歴を書き換える操作である。すでに複数人で共有しているブランチで実行すると、履歴が合わなくなる可能性があるため注意が必要である。

Gitの操作に慣れるまでは、GitHubのプルリクエストにある`Squash and merge`を使用する方が分かりやすい。

---

## スカッシュを使うとよい場面

スカッシュは、次のような場合に向いている。

- 細かい修正コミットが多数ある
- 1つの機能に関するコミットをまとめたい
- `main`ブランチの履歴を簡潔にしたい
- プルリクエスト単位で1つのコミットを残したい

一方で、途中の変更履歴を詳しく残したい場合は、通常のマージを使用する方法もある。

---

## まとめ

スカッシュとは、複数のコミットを1つのコミットにまとめる操作である。
...
スカッシュを利用すると、`main`ブランチのコミット履歴を簡潔で分かりやすい状態に保てる。
